...
// this component works but logic is in component whereas it should be in service
@Component({
  selector: 'my-grid',
  standalone: true,
...
export class GridSuccessComponent implements OnChanges{
  ...
  public ngOnChanges(): void {
    ...
    for (let row = 0; row < this.game.getGrid().rows; row++) {
      for (let col = 0; col < this.game.getGrid().cols; col++) {
        ...
        this.values[row][col] = this.game.getGrid();
      }
    }
  }
}
